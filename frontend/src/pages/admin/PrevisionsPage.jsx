    ...
    <div style={{
      ...
      <div>
        <div style={{ color: '#fca5a5', fontWeight: 600, fontSize: 14 }}>
          {t('fc_peak_alert')} {date}
        </div>
        <div style={{ color: '#94a3b8', fontSize: 12 }}>
          ~{Math.round(predicted)} {t('fc_orders_expected')}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ padding: '24px 28px', maxWidth: 1100 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
        ...
        <div style={{
          ...
          {t('fc_trend')} {tendancePos ? '▲' : '▼'} {Math.abs(tendance).toFixed(1)} {t('fc_orders_per_day')}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 28 }}>
        {[
          { label: t('fc_avg_30_days'),    value: data?.moyenne_historique ?? '—', icon: '📊', color: '#22c55e' },
          { label: t('fc_next_week'),      value: data?.total_semaine_prochaine ?? '—', icon: '🔮', color: '#16a34a' },
          { label: t('fc_peaks_detected'), value: alertes.length, icon: '⚠️', color: alertes.length ? '#ef4444' : '#22c55e' },
        ].map((k, i) => (
          <div key={i} style={{
      ...
      <div style={{
        ...
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={combined} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <Line
              type="monotone" dataKey="commandes" name={t('drv_history')}
              stroke="#22c55e" strokeWidth={2} dot={false}
              connectNulls={false}
            />
            <Line
              type="monotone" dataKey="prevision" name={t('fc_title')}
            ...
            {historique.length > 0 && (
              <ReferenceLine
                ...
                strokeDasharray="4 4"
                label={{ value: t('fc_today'), fill: '#94a3b8', fontSize: 11 }}
              />
            )}
            {/* Marqueurs pics */}
            {alertes.map((a, i) => (
