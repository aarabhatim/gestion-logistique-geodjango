...
export default function ZonesPage() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [z, t] = await Promise.all([zonesApi.list(), transporteursApi.adminListe()]);
      setZones(z.data?.results || z.data || []);
      setTransporteurs(t.data?.results || t.data || []);
    } catch { setZones([]); }
  ...
  const handleSave = async () => {
    try {
      if (selected?.id) { await zonesApi.update(selected.id, form); showToast('Zone mise à jour'); }
      else { await zonesApi.create(form); showToast('Zone créée'); }
      setShowForm(false); setForm({ nom: '', description: '', tarif_base: 0, tarif_km_supplementaire: 0, couleur: '#3b82f6', actif: true });
      fetchAll();
  ...
  const handleDelete = async (id) => {
    ...
    try { await zonesApi.delete(id); showToast('Zone supprimée'); fetchAll(); } catch { showToast('Erreur', 'error'); }
  };

  const handleAssign = async (zoneId, tid) => {
    try { await zonesApi.assignerTransporteur(zoneId, tid); showToast('Transporteur assigné'); fetchAll(); }
    catch { showToast('Erreur', 'error'); }
  };
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-header animate-fade-in" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h2 className="page-title text-gradient" style={{ display: 'flex', alignItems: 'center', gap: 10 }}><MapPin size={22} /> Zones de livraison</h2>
          <p className="page-subtitle">{zones.length} zones configurées</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {loading ? <div style={{ padding: '2rem', textAlign: 'center' }}><div className="spinner" style={{ margin: 'auto' }} /></div> : zones.map(z => (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>
                    Tarif base: <b>{z.tarif_base} DH</b> · +{z.tarif_km_supplementaire}/km
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 4, marginTop: 3 }}>
                    <Users size={10} /> {z.transporteurs_count || 0} transporteur{(z.transporteurs_count || 0) !== 1 ? 's' : ''}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 4 }}>
                  <button className="btn btn-secondary" style={{ padding: '4px 8px' }} onClick={e => { e.stopPropagation(); setForm({ nom: z.nom, description: z.description || '', tarif_base: z.tarif_base, tarif_km_supplementaire: z.tarif_km_supplementaire, couleur: z.couleur, actif: z.actif }); setSelected(z); setShowForm(true); }}><Edit2 size={12} /></button>
          ...
          <MapContainer center={[33.589886, -7.603869]} zoom={11} style={{ height: 500 }}>
            ...
            {zones.map(z => z.polygone?.coordinates && (
              <Polygon key={z.id}
                ...
                <Popup><b>{z.nom}</b><br />Tarif: {z.tarif_base} DH</Popup>
              </Polygon>
            ))}
          </MapContainer>
        </div>
      ...
      {selected && !showForm && (
        <div className="glass-card animate-fade-in" style={{ marginTop: '1.25rem', padding: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
            <h3 style={{ margin: 0 }}>Assigner des transporteurs — {selected.nom}</h3>
            <button className="btn btn-secondary" style={{ padding: '4px 8px' }} onClick={() => setSelected(null)}><X size={14} /></button>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {transporteurs.slice(0, 20).map(t => (
              <button key={t.id} className="btn btn-secondary" style={{ fontSize: 12 }} onClick={() => handleAssign(selected.id, t.id)}>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="glass-card animate-fade-in" style={{ width: 420, padding: '1.5rem' }}>
            <h3 style={{ margin: '0 0 1rem', fontSize: 15 }}>{selected ? 'Modifier' : 'Nouvelle'} zone</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[['nom', 'Nom de la zone'], ['description', 'Description']].map(([key, label]) => (
                <div key={key}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Tarif base (DH)</label>
                ...
                <div>
                  <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>+DH/km</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Couleur</label>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 8 }}>
                ...
                <button className="btn btn-primary" onClick={handleSave} style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Save size={14} /> Enregistrer</button>
              </div>
            </div>
          </div>
        </div>
