  ...
  return (
    <div style={{ maxWidth: 1600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: T.text2, marginTop: 4 }}>
            DeliverMap — indicateurs en temps réel
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
        ...
        <KpiCard delay={0.05} loading={loading} icon={Truck}      label="En cours"
          value={(kpis.commandes_en_cours || 0).toLocaleString()}
          sub={`Taux ${kpis.taux_livraison || 0}%`}
          color={T.accent} trend={-2.1} />
        <KpiCard delay={0.10} loading={loading} icon={Users}      label="Clients inscrits"
          value={(kpis.clients_total || 0).toLocaleString()}
          sub="Total plateforme"
          color={T.info} trend={5.2} />
        <KpiCard delay={0.15} loading={loading} icon={TrendingUp} label="Chiffre d'affaires"
          value={`${Math.round(kpis.ca_total || 0).toLocaleString()} MAD`}
          sub={`${Math.round(kpis.ca_mois || 0).toLocaleString()} MAD ce mois`}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Évolution des commandes</span>
              </div>
              <div style={{ display: 'flex', gap: 16 }}>
                ...
                <div>
                  <span style={{ fontSize: 11, color: T.text2 }}>CA total</span>
                  <div style={{ fontSize: 18, fontWeight: 800, color: T.text }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          style={CARD}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Efficacité flotte</span>
            <span style={{ padding: '4px 10px', borderRadius: 20, background: 'rgba(34,197,94,0.1)', color: T.primary, fontSize: 11, fontWeight: 700 }}>Actuel</span>
          </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 12, color: T.text2 }}>Taux global</span>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Véhicules en route</span>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 12, color: T.danger, marginTop: 4, display: 'flex', alignItems: 'center', gap: 3 }}>
              <ArrowDownRight size={13} /> -1.6% vs mois dernier
            </div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Activités récentes</span>
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Top Boutiques</span>
            </div>
            <span style={{ padding: '4px 10px', borderRadius: 20, background: 'rgba(34,197,94,0.1)', color: T.primary, fontSize: 11, fontWeight: 700 }}>Par CA</span>
          </div>
          {(data?.top_fondateurs || []).length === 0
            ? <p style={{ fontSize: 13, color: T.text2 }}>Aucune donnée disponible</p>
        ...
        <div style={CARD}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Top Transporteurs</span>
            </div>
            <span style={{ padding: '4px 10px', borderRadius: 20, background: 'rgba(245,158,11,0.1)', color: T.accent, fontSize: 11, fontWeight: 700 }}>Par livraisons</span>
          </div>
            ...
            : (data?.top_transporteurs || []).map((t, i) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: T.text }}>{t.user__first_name} {t.user__last_name}</div>
                    <div style={{ fontSize: 10.5, color: T.text2 }}>{t.vehicule_type}</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 15, fontWeight: 800, color: T.accent }}>{t.nombre_livraisons}</div>
                  <div style={{ fontSize: 10, color: T.text2 }}>livraisons</div>
                </div>
              </div>
            ))
          }
