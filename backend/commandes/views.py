...
class CommandeStatutView(APIView):
    ...
    def post(self, request, pk, action):
        ...
        if action == 'annuler' and commande.statut not in ('LIVREE', 'ANNULEE'):
            # Le CLIENT ne peut PAS annuler quand la commande est deja EN_ROUTE
            # (le livreur est en route, ce serait abusif)
            if role == 'CLIENT' and commande.statut == 'EN_ROUTE':
                return Response(
                    {'error': "Vous ne pouvez plus annuler : votre commande est déjà en cours de livraison. "
                              "Contactez le support ou signalez un problème si nécessaire."},
